...
def wait_for_reply(session_id, initial_count, max_wait=90, check_interval=3):
    """等待 AI 回复"""
...
def main():
    ...
    print(f"   官网链接: {base_url}/zh/chat/{session_id}")
    ...
    ai_reply = wait_for_reply(session_id, initial_count, max_wait=90, check_interval=3)
    ...
    if ai_reply:
        ...
        filename = f"new_session_reply_{timestamp}.txt"
        ...
        print(f"\n💾 回复已保存到: {filename}")
        print(f"\n🎉 测试成功！新会话模式可以获取 AI 回复！")
    else:
        print("\n⚠️  未能获取到 AI 回复")
        print("💡 建议:")
        print(f"   1. 到 BaziAI 官网查看: {base_url}/zh/chat/{session_id}")
        print("   2. 检查 Cookie 是否有效")
        print("   3. 尝试增加等待时间")
